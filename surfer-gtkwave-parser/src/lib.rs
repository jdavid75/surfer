...
/// A line in a GTKWave dumpfile.
///
/// Not all directives are contained on a single line:
///
/// - Some directives can be preceded by a [Flags] integer on its own line.
/// - Trace directives can be preceded by a [Color] directive, in which case
///   the color directive is stored inside the variant, like [`Directive::Trace::color`].
///
/// Since we only care about directives that make sense to surfer, some directives
/// will be ignored.
///
/// Most directives have the format `[directive] <arguments...>`. The expected format
/// of each directive can be found in the variant's documentation comment.
#[derive(Debug)]
pub enum Directive {
    /// `[dumpfile] <path>`. Path to wave file to open.
    Dumpfile(String), // ignore dumpfile_mtime and dumpfile_size
    /// `*<arguments>`
    Markers,
    /// `<path>`. Add a single variable.
    Trace {
        path: Path,
        color: Option<Color>,
        flags: Option<Flags>,
    },
    /// `#{<path>} <paths...>`. Add a variable that expands into more variables.
    TraceMany {
        path: Path,
        rest: Vec<Path>,
        color: Option<Color>,
    ...
    /// `-<name>` preceded by a grp_begin flag. Open a group.
    OpenGroup { name: String, flags: Option<Flags> },
    /// `-<name>` preceded by a grp_end flag. Close a group.
    CloseGroup { name: String, flags: Option<Flags> },
    /// `-` preceded by a blank flag. Insert a blank signal with a comment as its name.
    Comment { text: String, flags: Option<Flags> },
}
...
impl<'s> Parser<'s> {
    ...
    fn try_signal(&mut self) -> Result<Option<Directive>> {
        ...
        if line.starts_with('-') && flags.is_some_and(|flags| flags.contains(Flags::grp_begin)) {
            self.open_group(flags).map(Some)
        } else if line.starts_with('-') && flags.is_some_and(|flags| flags.contains(Flags::grp_end))
        {
            self.close_group(flags).map(Some)
        } else if line.starts_with('-') && flags.is_some_and(|flags| flags.contains(Flags::blank)) {
            self.comment(flags).map(Some)
        } else if line.starts_with('#') {
    ...
    fn open_group(&mut self, flags: Option<Flags>) -> Result<Directive> {
        // trim `-`
        let name = self.peek_line()?[1..].to_string();
        Ok(Directive::OpenGroup { name, flags })
    }

    fn close_group(&mut self, flags: Option<Flags>) -> Result<Directive> {
        // trim `-`
        let name = self.peek_line()?[1..].to_string();
        Ok(Directive::CloseGroup { name, flags })
    }

    fn comment(&mut self, flags: Option<Flags>) -> Result<Directive> {
        // trim `-`
        let text = self.peek_line()?[1..].to_string();
        Ok(Directive::Comment { text, flags })
    }
    ...
    fn trace_path(&self, path: &str) -> Result<Path> {
        let Some((path, bit_range)) = path.split_once('[') else {
